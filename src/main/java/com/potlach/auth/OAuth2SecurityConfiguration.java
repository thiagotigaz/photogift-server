...
import com.potlach.util.JPAUserDetailsService;
...
public class OAuth2SecurityConfiguration {
	...
	protected static class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
		...
		public void configure(WebSecurity web) throws Exception {
			web.ignoring().antMatchers("/insecure/**");
		}
	...
	protected static class ResourceServer extends
		...
		public void configure(HttpSecurity http) throws Exception {
			...
			http.authorizeRequests().antMatchers("/insecure/**").permitAll();
	...
	protected static class OAuth2Config extends
		...
		public OAuth2Config() throws Exception {
			...
			ClientDetailsService csvc = new InMemoryClientDetailsServiceBuilder()
					...
					.scopes("read","write").resourceIds("user")
					.and()
					// Create a second client that only has "read" access to the
					// video service
					.withClient("mobileReader").authorizedGrantTypes("password")
					.authorities("ROLE_CLIENT")
					.scopes("read").resourceIds("user")
					.accessTokenValiditySeconds(15).and().build();

			// Create a series of hard-coded users. 
			/*UserDetailsService svc = new InMemoryUserDetailsManager(
					Arrays.asList(
							User.create(1l,"admin", "pass", "ADMIN", "USER"),
							User.create(2l,"user0", "pass", "USER"),
							User.create(3l,"user1", "pass", "USER"),
							User.create(4l,"user2", "pass", "USER"),
							User.create(5l,"user3", "pass", "USER"),
							User.create(6l,"user4", "pass", "USER"),
							User.create(7l,"user5", "pass", "USER")));
*/
